...
export default function HeroSlide({ imgSrc, title }: Props) {
    return (
        <div className="h-full w-full relative">
            <Image className="h-full object-cover" src={imgSrc} alt="" />
            <div className="absolute top-0 left-1/2 h-full w-full max-w-6xl 
                        flex flex-col justify-between -translate-x-1/2 px-12 py-20">
                <h1
                    className="text-3xl text-white w-1/2 text-left font-semibold"
                >
                    {title}
                </h1>
                <h2
